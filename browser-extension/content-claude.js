...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'fillPrompt') {
    fillClaudePrompt(request.prompt);
...
function showNotification() {
  ...
  notification.style.cssText = `
    ...
    top: 20px;
    right: 20px;
    background: linear-gradient(135deg, #d97757 0%, #c96647 100%);
    color: white;
    padding: 20px 24px;
    border-radius: 16px;
...
function fillClaudePrompt(prompt) {
  setTimeout(() => {
    let targetElement = null;
    
    // Strategy 1: Look for ProseMirror editor (Claude's editor)
    const contentEditables = document.querySelectorAll('[contenteditable="true"]');
    for (const elem of contentEditables) {
      if (elem.classList.contains('ProseMirror') || 
          elem.closest('.ProseMirror') ||
    ...
    // Strategy 2: Look for any large contenteditable
    if (!targetElement) {
      let largestArea = 0;
      for (const elem of contentEditables) {
        const rect = elem.getBoundingClientRect();
    ...
    // Strategy 3: Look for textarea fallback
    if (!targetElement) {
    ...
    if (targetElement) {
      console.log('Claude: Found target element:', targetElement);
      targetElement.focus();
      ...
      setTimeout(() => {
        if (targetElement.tagName === 'TEXTAREA') {
          ...
          // Create a more comprehensive input event
          ...
          targetElement.dispatchEvent(new CompositionEvent('compositionend', { bubbles: true, data: prompt }));
          targetElement.dispatchEvent(new Event('change', { bubbles: true }));
        }
      ...
      console.log('Claude: Could not find input, copying to clipboard');
