...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'fillPrompt') {
    fillCopilotPrompt(request.prompt);
...
function fillCopilotPrompt(prompt) {
  ...
  setTimeout(() => {
    ...
    // Strategy 1: Look for textarea with "Message" or "Copilot" in placeholder
    const textareas = document.querySelectorAll('textarea');
    for (const textarea of textareas) {
      if (textarea.placeholder && 
          (textarea.placeholder.toLowerCase().includes('message') || 
           textarea.placeholder.toLowerCase().includes('copilot') ||
    ...
    // Strategy 2: Look for contenteditable with role="textbox"
    if (!targetElement) {
      const contentEditables = document.querySelectorAll('[contenteditable="true"]');
      for (const elem of contentEditables) {
        if (elem.getAttribute('role') === 'textbox' || 
            elem.getAttribute('aria-label')?.toLowerCase().includes('message')) {
          targetElement = elem;
          break;
        }
      }
    ...
    // Strategy 3: Look for any visible textarea
    ...
    // Strategy 4: Look for input with specific classes or IDs
    if (!targetElement) {
      const inputs = document.querySelectorAll('input[type="text"], div[role="textbox"]');
    ...
    if (targetElement) {
      ...
      console.log('Could not find input element, copying to clipboard');
